...
2. check_vacancy()- returns the count of the compartments in which more than 50% of the seats 
   are vacant

Note : Compartment list is maintained as a linked list where data in each node refers to a 
       compartment.
'''
...
class Train:
    ...
    def check_vacancy(self):
        ...
        vacancy_count=0
        while temp!=None:
            compartment=temp.get_data()
            vacancy=compartment.get_total_seats()-compartment.get_no_of_passengers()
            vacancy_percentage=vacancy/compartment.get_total_seats()
            if vacancy_percentage>0.5:
                vacancy_count+=1
            temp=temp.get_next()
