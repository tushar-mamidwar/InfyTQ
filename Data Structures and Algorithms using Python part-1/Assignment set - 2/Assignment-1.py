...
2. In case of two consecutive occurrences of '*' or '/' , replace those two occurrences by a 
...
def create_new_sentence(word_list):
    ...
    while temp != None:
        current_character = temp.get_data()
        if current_character == "*" or current_character == "/":
            if previous_character == "*" or previous_character == "/":
                temp = temp.get_next()
                current_character = temp.get_data().upper()
                new_sentence += current_character
            else:
                new_sentence += " "
        else:
            new_sentence += current_character
