"""
Problem Statement
Write a python program to find the maximum value in a linked list and replace it with a 
given value.
...
def replace_maximum(number_list, new_value):
    ...
    maximum = 0
    while temp != None:
        if temp.get_data() > maximum:
            maximum = temp.get_data()
        temp = temp.get_next()
    max_node = number_list.find_node(maximum)
